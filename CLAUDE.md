...
- **Inbound email — LIVE & end-to-end verified 2026-06-24:** the webhook (`apps/public/app/api/webhooks/inbound-email/route.ts`) natively accepts **Resend Inbound** `email.received` events (Svix-signed, secret in `RESEND_WEBHOOK_SECRET` on uhs-public; body fetched from Resend's Received Emails API) and still accepts the dormant legacy Cloudflare Worker transport (`workers/inbound-email-router/`, bearer `INBOUND_WEBHOOK_SECRET`). Config: Resend receiving domain `replies.upstatehomecenter.com` (us-east-1, `receiving: enabled`, sending disabled = receive-only); GoDaddy **MX `replies` → `inbound-smtp.us-east-1.amazonaws.com` prio 10** (Resend marks the MX `verified`); Resend webhook → `https://upstatehomecenter.com/api/webhooks/inbound-email` (event `email.received`); `RESEND_WEBHOOK_SECRET` set on uhs-public prod+preview and redeployed. Verified: smoke test 4/4 + a real-reply E2E (reply to `replies+{token}@…` landed as an `inbound`/`email` `lead_messages` row). **⚠ Don't be alarmed that the Resend domain shows `status: failed` / DKIM `failed`** — receiving needs only the **verified MX**, not full domain verification; the `failed` DKIM is cosmetic for a receive-only domain (the record is correct in DNS; Resend just won't mark it verified). The real activation delay was ~hours of AWS SES receipt-rule provisioning (probes bounced until it finished), NOT the DKIM. Do **not** re-introduce Cloudflare Email Routing or move nameservers to "fix" inbound — Resend Inbound is the chosen transport and DNS stays on GoDaddy.
