...
export default async function HomeDetailPage({ params }: { params: Promise<Params> }) {
  ...
  const { data: home } = await supabase
    .from('public_homes')
    .select(
      'id, stock_no, name, model, type, beds, baths, beds_options, baths_options, sqft, width_ft, length_ft, year_built, construction, listed_price_cents, prices_hidden, starting_from, headline, description, matterport_url, on_lot_since, manufacturer_id, manufacturers(name)'
    )
    .eq('stock_no', decodeURIComponent(stock))
    .maybeSingle();
